...
## Flows: where does the majority of trips goes from every district

## Filter if Location destiny is same as origin

PUDO_intra  <-  filter(data, PULocationID == DOLocationID)[,c(8,9)]
PUDO_inter  <-  filter(data, PULocationID != DOLocationID)[,c(8,9)] %>%
    group_by(PULocationID) %>%
    summarise(no_rows = length(PULocationID))

PUDO  <- melt(table(data$PULocationID, data$DOLocationID))
names(PUDO)  <-  c("PU","DO","Freq")
    


## Frequency of combinations of PU and DO
...
df <- data %>%
    group_by(PULocationID,DOLocationID) %>%
    summarise(Freq = length(PULocationID))

df[df$PULocationID == 4 & df$DOLocationID == 7, "Freq"] 

## I need the centroids of each polygon.


sf_cent <- st_centroid(spZones)
plot(sf_cent[1], cex=0.5)
 
ggplot() + 
  geom_sf(data = sf_cent, fill = 'white') +
  geom_sf(data = sf_cent %>% st_as_sf, color = 'blue') + 
    geom_sf(data = sf_cent, color = 'red')

xy  <- st_coordinates(st_centroid(spZones))
xy  <-  as.data.frame(xy)
names(xy)  <-  c("Xor", "Yor")
xy$PULocationID  <-  seq(1:263)

xydest  <- st_coordinates(st_centroid(spZones))
xydest  <-  as.data.frame(xydest)
names(xydest)  <-  c("Xdest", "Ydest")
xydest$DOLocationID  <-  seq(1:263)

xyor  <-  merge(df, xy, by='PULocationID')
xydest  <-  merge(xyor, xydest, by='DOLocationID')
...
ggplot(xydest[which(xydest$Freq> 50),], aes(Xor, Yor))+
                                        #The next line tells ggplot that we wish to plot line segments. The "alpha=" is line transparency and used below +
   geom_segment(aes(x=Xor, y=Yor,xend=Xdest, yend=Ydest, alpha=Freq), col="white")+
