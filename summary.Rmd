...
There is also few amount of data records that show trips starting at Queens and Brooklyn, ending in a similar amount in Manhattan, Brooklyn or Queens. So that means that the **trip flows** are inside or to Manhattan.
...
The above picture shows that the are few differences on the data distribution of each variable regardless the *pick up* and *drop off area*. Among areas, we see how the borough of Manhattan is the one with lower fare, tip amount, trip distance and trip duration. We also see that the borough of staten Island is the one that presents higher values for all the variables, but we cannot forget that it is a borough with few data records. 
...
The figure shows that for the fare, the tip and the trip distance, the sunday is different from the rest of the week, with higher median values. For the trip duration, it seems to be the central days of the weeks the ones with higher values.
...
ggpairs(df[, c(5,11,29,17,18)], aes(colour = df$PU_servicezone, alpha = 0.4), upper = list(continuous = wrap("cor", size=2.5)),lower = list(continuous = wrap(funcVal="points", alpha = 1, size=0.5)))
```

In this case, the % tip is centered in 20-25% when the taxi is picked up in the Yellow or Boro Zone. In the case of the airports, we see a bimodal distribution, with a second maximum in 25%.
...
Most of the locations are around 20to 23 but few of them present values below 15 and above 25.
...
Only the location in the Staten Islands presents trip length around 30 and clearly the airports are around 20 miles. The rest are below 10 in general.
...
The same map depdending on the drop off is a bit different. It seems to be a cluster centered in Manhattan with the lower values and largest values goes to the margins of the figure. This has sense if we consider that most of the trips comes from the Manhattan area, so the distance increases for the locations far from that borough.
